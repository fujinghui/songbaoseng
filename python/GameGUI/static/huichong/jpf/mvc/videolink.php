<?php
	...
	class VideoLinkOP{
		public function gets($sql){
			$list = Array();
			$result = $sql->query("select * from video_link2");
		...
		public function getbyid($id, $sql){
			$result = $sql->query("select * from video_link2 video_link_id=$id");
			if($result && $result->num_rows > 0)
			{
				while($row=$result->fetch_assoc())
				{
		...
		public function update($v, $sql){
			if($sql->query("update video_link2 set video_link_url='{$v->video_link_url}',video_link_img_src='{$v->video_link_img_src}', video_link_context='{$v->video_link_context}' where video_link_id={$v->video_link_id}"))
			{
				return true;
			}
			return false;
		...
		public function insert($v, $sql){
			if($sql->query("insert into video_link2 values(null, '{$v->video_link_url}', '{$v->video_link_img_src}', '{$v->video_link_context}')"))
				return true;
			return false;
		}
		public function delete($v, $sql){
			$result = $sql->query("select * from video_link2 where video_link_id={$v->video_link_id}");
			if($result && $result->num_rows > 0)
			{
				...
				if($sql->query("delete from video_link2 where video_link_id={$v->video_link_id}"))
					return true;
			}
			return false;
		}
