...
<html>
	<head>
		...
		<script language="javascript">
			...
			function SetVideoLink(){
				$.post("op/getvideolinks.php", {}, function(data){
					...
					for(var i = 0; i < s.count; i ++)
					{
						...
						show_text += "<textarea style='height:100px;width:100px;' class='form-control' placeholder='请输入视频链接'>";
						...
						show_text += "</textarea>";
						show_text += "<textarea style='height:100px;width:100px;' class='form-control' placeholder='请输入提示的文字'>";
			...
			function GetWordBlog(){
				$.post("get_word_blogs.php", {}, function(data){
					console.log(data);
					var s = JSON.parse(data);
					if(s.count == 0)
					{
						$("#show_tools").html("暂无留言！");
						return;
					}
					var show_text = "<div style='margin-top:10px;'><ul class='list-group'>";
					for(var i = 0; i < s.count; i ++)
					{
						show_text += "<li class='list-group-item'><a href='#'>";
						show_text += s.data[i].context;
						show_text += "</a><a href='javascript:void(0)' class='pull-right' onclick='DeleteWordBlog(this,"+s.data[i].word_id+")'>删除</a><br />";			//实现点击删除
						show_text += "<span style='color:#999;'>"+s.data[i].user_name+"<small style='color:#f00'>&nbsp;&nbsp;联系方式："+s.data[i].contact_way+"</small></span>";
						show_text += "<span class='pull-right'>"+s.data[i].word_date+"</span>";
						show_text += "</li>";
					}
					show_text += "</ul></div>";
				...
				<nav class="navbar navbar-default" role="navigation" style="box-shadow:5px 10px 0px rgba(100, 100, 100, 0.1)">
					<div class="container-fluid">
						<div class="navbar-header">
							...
							<a class="navbar-brand" href="index.php">狗狗营视频管理</a>
						</div>
					...
					<div class="row">
						<div class="span1">
							<ul class="nav nav-list slide" >
								<li class="nav-header"><h3 style="color:#999">宠物训练营管理</h3></li>
								
								<li><a href="javascript:void(0)" onclick="SetVideoLink()">添加视频链接</a></li>
								<li><a href="javascript:void(0)" onclick="GetWordBlog()">留言管理</a></li>
							</ul>
						</div>
					</div>
				</div>
